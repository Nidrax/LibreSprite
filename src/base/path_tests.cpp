...
{
  // Valid filenames
  EXPECT_TRUE(is_valid_filename("image.png"));
  EXPECT_TRUE(is_valid_filename("my_file.txt"));
  EXPECT_TRUE(is_valid_filename("sprite-01.ase"));
  EXPECT_TRUE(is_valid_filename("file name with spaces.png"));
  EXPECT_TRUE(is_valid_filename("file@#$%&()[];',=+-.png"));

  // Invalid: empty
  EXPECT_FALSE(is_valid_filename(""));

#ifdef _WIN32
  // Windows illegal characters: < > : " / \ | ? *
  EXPECT_FALSE(is_valid_filename("file<name.png"));
  EXPECT_FALSE(is_valid_filename("file>name.png"));
  EXPECT_FALSE(is_valid_filename("file:name.png"));
  EXPECT_FALSE(is_valid_filename("file\"name.png"));
  EXPECT_FALSE(is_valid_filename("file/name.png"));
  EXPECT_FALSE(is_valid_filename("file\\name.png"));
  EXPECT_FALSE(is_valid_filename("file|name.png"));
  EXPECT_FALSE(is_valid_filename("file?name.png"));
  EXPECT_FALSE(is_valid_filename("file*name.png"));

  // Windows reserved names
  EXPECT_FALSE(is_valid_filename("CON.png"));
  EXPECT_FALSE(is_valid_filename("PRN.txt"));
  EXPECT_FALSE(is_valid_filename("AUX"));
  EXPECT_FALSE(is_valid_filename("NUL.dat"));
  EXPECT_FALSE(is_valid_filename("COM1.txt"));
  EXPECT_FALSE(is_valid_filename("LPT1.png"));
  EXPECT_FALSE(is_valid_filename("con.png")); // case-insensitive

  // Windows: cannot end with period or space
  EXPECT_FALSE(is_valid_filename("filename."));
  EXPECT_FALSE(is_valid_filename("filename "));
#else
  // Unix-like systems: only '/' is illegal (besides null)
  EXPECT_FALSE(is_valid_filename("file/name.png"));
  
  // These are valid on Unix but not Windows
  EXPECT_TRUE(is_valid_filename("file:name.png"));
  EXPECT_TRUE(is_valid_filename("file<name.png"));
  EXPECT_TRUE(is_valid_filename("file>name.png"));
  EXPECT_TRUE(is_valid_filename("CON.png"));
  EXPECT_TRUE(is_valid_filename("filename."));
#endif
}

TEST(Path, GetFilenameValidationError)
{
  // Valid filenames should return empty string
  EXPECT_EQ("", get_filename_validation_error("valid_file.png"));
  EXPECT_EQ("", get_filename_validation_error("another-file.txt"));

  // Empty filename
  EXPECT_NE("", get_filename_validation_error(""));

#ifdef _WIN32
  // Windows illegal characters
  EXPECT_NE("", get_filename_validation_error("file:name.png"));
  EXPECT_NE("", get_filename_validation_error("file<name.png"));
  
  // Windows reserved names
  EXPECT_NE("", get_filename_validation_error("CON.png"));
  EXPECT_NE("", get_filename_validation_error("NUL"));
  
  // Trailing period or space
  EXPECT_NE("", get_filename_validation_error("filename."));
  EXPECT_NE("", get_filename_validation_error("filename "));
#else
  // Unix: only '/' is illegal
  EXPECT_NE("", get_filename_validation_error("file/name.png"));
#endif
}
